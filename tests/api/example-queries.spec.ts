...
test.describe('Chat API - Example Queries Batch', () => {
  ...
  exampleQueries.forEach((example) => {
    test(`should handle example query: ${example.id} - "${example.text.split('\n')[0]}..."`, async ({ request }) => {
      ...
      expect(response.status(), `Failed on query: ${example.text}`).toBe(200);
      ...
      if (example.category === 'trips') {
        ...
        const tripCalls = data.toolCalls?.filter((tc: any) => tc.toolName === 'findTrips');
        expect(tripCalls?.length || 0, `Expected trip tool call for: ${example.text}`).toBeGreaterThan(0);
      }
      
      if (example.category === 'weather') {
        // Should have weather or snow tool calls
        const weatherCalls = data.toolCalls?.filter((tc: any) => 
          tc.toolName === 'getWeather' || tc.toolName === 'getSnowConditions'
        );
        expect(weatherCalls?.length || 0, `Expected weather tool call for: ${example.text}`).toBeGreaterThan(0);
      }
      
      if (example.category === 'stations') {
        // Should have place info or events tool calls
        const stationCalls = data.toolCalls?.filter((tc: any) => 
          tc.toolName === 'getPlaceEvents' || tc.toolName === 'findStopPlacesByName'
        );
        expect(stationCalls?.length || 0, `Expected station tool call for: ${example.text}`).toBeGreaterThan(0);
      }

      if (example.category === 'markdown') {
        // Markdown queries are often multi-part or complex, check if we got ANY tool calls
        expect(data.toolCalls?.length || 0, `Expected at least one tool call for complex markdown query: ${example.text}`).toBeGreaterThan(0);
      }
    });
  });
