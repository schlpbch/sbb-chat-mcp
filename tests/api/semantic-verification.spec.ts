...
const testCases = [
  ...
  { query: 'Chur to St. Gallen', type: 'trip', origin: 'Chur', dest: 'St. Gallen' },
  { query: 'Interlaken Ost to Grindelwald', type: 'trip', origin: 'Interlaken', dest: 'Grindelwald' },
  { query: 'Zermatt to Visp', type: 'trip', origin: 'Zermatt', dest: 'Visp' },
  { query: 'Lausanne to Montreux', type: 'trip', origin: 'Lausanne', dest: 'Montreux' },
  { query: 'Fribourg to Neuchatel', type: 'trip', origin: 'Fribourg', dest: 'Neuchatel' },
  { query: 'Olten to Aarau', type: 'trip', origin: 'Olten', dest: 'Aarau' },
  { query: 'Biel/Bienne to Solothurn', type: 'trip', origin: 'Biel', dest: 'Solothurn' },

  // --- MULTILINGUAL (German) ---
  { query: 'Zug von Bern nach Zürich', type: 'trip', origin: 'Bern', dest: 'Zürich', lang: 'de' },
  { query: 'Verbindung von Basel nach Luzern', type: 'trip', origin: 'Basel', dest: 'Luzern', lang: 'de' },
  { query: 'Reise von Chur nach Arosa', type: 'trip', origin: 'Chur', dest: 'Arosa', lang: 'de' },
  { query: 'Abfahrt in Thun', type: 'station', queryStr: 'Thun', lang: 'de' },
  { query: 'Wetter in Bern', type: 'weather', location: 'Bern', lang: 'de' },
  { query: 'Zürich bis Genf', type: 'trip', origin: 'Zürich', dest: 'Genf', lang: 'de' },

  // --- MULTILINGUAL (French) ---
  { query: 'Train de Genève à Lausanne', type: 'trip', origin: 'Genève', dest: 'Lausanne', lang: 'fr' },
  { query: 'Aller de Fribourg à Berne', type: 'trip', origin: 'Fribourg', dest: 'Berne', lang: 'fr' },
  { query: 'Horaires depuis Sion', type: 'station', queryStr: 'Sion', lang: 'fr' },
  { query: 'Météo à Montreux', type: 'weather', location: 'Montreux', lang: 'fr' },
  { query: 'Connexion de Neuchâtel à Yverdon', type: 'trip', origin: 'Neuchâtel', dest: 'Yverdon', lang: 'fr' },

  // --- MULTILINGUAL (Italian) ---
  // Note: Italian parsing might be basic, we test what we expect to work or fallback
  { query: 'Treno da Lugano a Bellinzona', type: 'trip', origin: 'Lugano', dest: 'Bellinzona', lang: 'it' },
  
  // --- DATE & TIME ---
  { query: 'Zurich to Bern at 08:00', type: 'trip', origin: 'Zurich', dest: 'Bern', time: '08:00' },
  { query: 'Basel to Geneva tomorrow', type: 'trip', origin: 'Basel', dest: 'Geneva', checkDate: 'tomorrow' },
  { query: 'Luzern to Lugano at 14:30', type: 'trip', origin: 'Luzern', dest: 'Lugano', time: '14:30' },
  { query: 'Chur to Zurich 10:00', type: 'trip', origin: 'Chur', dest: 'Zurich', time: '10:00' },

  // --- STATION BOARD ---
  { query: 'Departures from Thun', type: 'station', queryStr: 'Thun' },
  { query: 'Arrivals in Zurich HB', type: 'station', queryStr: 'Zurich', eventType: 'arrivals' },
  { query: 'Station board Bern', type: 'station', queryStr: 'Bern' },
  { query: 'Show me trains from Geneva Airport', type: 'station', queryStr: 'Geneva' }, 
  ...
  for (const tc of testCases) {
    test(`Scenario: "${tc.query}"`, async ({ request }) => {
      ...
      if (response.status() === 500) {
          console.log(`[500 ERROR] details for "${tc.query}":`, await response.text());
      }
      ...
      if (body.toolCalls && body.toolCalls.length > 0) {
          ...
          if (tc.type === 'trip') {
            const tool = body.toolCalls.find((t: any) => t.toolName === 'findTrips');
            ...
            if (tool) {
                if (tc.origin) expect(tool.params.origin).toMatch(new RegExp(tc.origin.replace('.', '\\.'), 'i'));
                if (tc.dest) expect(tool.params.destination).toMatch(new RegExp(tc.dest.replace('.', '\\.'), 'i'));
                if (tc.time) {
                ...
                // If findTrips missing, check debug intent or error
                 console.log(`[WARN] No findTrips for "${tc.query}". Tools:`, body.toolCalls.map((t:any)=>t.toolName));
            }
          } 
            ...
            expect(relevantCall, `Expected station tool for "${tc.query}"`).toBeDefined();
            ...
            if (eventsCall && tc.eventType) {
              ...
              const tool = body.toolCalls.find((t: any) => t.toolName === 'getWeather' || t.toolName === 'getSnowConditions');
              expect(tool, `Expected weather tool for "${tc.query}"`).toBeDefined();
              if (tc.location) expect(tool.params.location).toMatch(new RegExp(tc.location.replace('.', '\\.'), 'i'));
          }
      } else {
          // No tools called? 
