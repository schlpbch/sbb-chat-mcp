...
import { Globe, Accessibility, Footprints, ArrowUpDown } from 'lucide-react';
...
import type { ShareableStation } from '@/lib/shareUtils';
...
export default function StationCard({ data, language }: StationCardProps) {
  ...
  const getCountryFlag = (code?: string) => {
    // Return country code text instead of emoji flags
    return code || 'INT';
  };

  const shareableStation: ShareableStation = {
    type: 'station',
    name: name || t.station.unknownStation,
    id,
  };

  return (
    <article
      className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md hover:shadow-lg transition-all duration-200 hover:border-blue-500 dark:hover:border-blue-400"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {services.slice(0, 3).map((service, idx) => (
                  <span
                    key={idx}
                    className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[9px] rounded"
                  >
                    {service}
                  </span>
                ))}
