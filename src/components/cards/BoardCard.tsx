...
export default function BoardCard({ data }: BoardCardProps) {
  ...
  return (
    <article
      className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md hover:shadow-lg transition-all duration-200 hover:border-purple-500 dark:hover:border-purple-500"
      ...
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        {connections.length > 0 ? (
          connections.slice(0, 5).map((conn, idx) => (
            <div
              key={idx}
              className="p-2 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2 flex-1 min-w-0">
                  ...
                  <div className="w-12 shrink-0">
                    ...
                    {conn.delay && (
                      <span className="text-xs px-1 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded">
                        {conn.delay}
                      </span>
                    )}
                  </div>
                  ...
                  {conn.platform && (
                    <div className="text-right shrink-0">
                      <p className="text-xs text-gray-500 dark:text-gray-400">Pl.</p>
          ...
          <div className="p-6 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">No {isDeparture ? 'departures' : 'arrivals'}</p>
          </div>
        )}
      </div>
