...
export default function FormationCard({ data, language }: FormationCardProps) {
  ...
  // 2. Under formations[key].compound_train
    ...
    if (keys.length > 0) {
      ...
      trainUnits =
        f.compound_train?.train_elements ||
  ...
  return (
    <article className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md hover:shadow-lg transition-all duration-200">
      <CardHeader
            ...
            <div key={uIdx} className="relative">
              ...
              <div className="flex items-center justify-between mb-2 px-1">
                <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase">
                  {t.formation.unit} {uIdx + 1}
                </span>
                {unit.operationalType && (
              ...
              <div className="flex items-end space-x-1 overflow-x-auto pb-4 scrollbar-hide">
                {(
                  ...
                  unit.elements ||
                  []
                )?.map((wagon: any, wIdx: number) => {
                  const is1st =
                    wagon.firstClass === true ||
                    wagon.class === '1' ||
                    wagon.class === 1;
                  const sector = wagon.sector || wagon.platform_sector || '';
                  const hasRestaurant =
                    wagon.type?.includes('WR') ||
                    wagon.notices?.some(
                      (n: any) => n.name === 'BE' || n.name === 'WR'
                    );
                  const isQuiet = wagon.notices?.some(
                    (n: any) => n.name === 'RZ'
                  );
                  const isFamily = wagon.notices?.some(
                    (n: any) => n.name === 'FA'
                  );
                  ...
                  return (
                    <div key={wIdx} className="flex flex-col items-center">
                      ...
                      <div
                        ...
                        title={`${
                          is1st
                            ? t.formation.firstClass
                            : t.formation.secondClass
                        } - ${t.formation.wagon} ${wagon.number || ''}`}
                      >
                        {/* Status Bar */}
                        <div
                      ...
                      {/* Wagon Number */}
                      <span className="text-[9px] text-gray-500 dark:text-gray-400 mt-1 font-medium">
                        #{wagon.number || wIdx + 1}
                      </span>
                    </div>
                  );
      ...
      {/* Legend */}
      <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 flex items-center justify-center space-x-4 text-[10px] text-gray-500 dark:text-gray-400">
        <div className="flex items-center">
          <div className="w-2 h-2 bg-yellow-400 dark:bg-yellow-500 rounded-full mr-1" />{' '}
          {t.formation.firstClass}
        </div>
        <div className="flex items-center">
          <div className="w-2 h-2 bg-gray-400 dark:bg-gray-600 rounded-full mr-1" />{' '}
          {t.formation.secondClass}
        </div>
        <div className="flex items-center">
          <span>🍴</span> {t.formation.restaurant}
        </div>
        <div className="flex items-center">
          <span>🤫</span> {t.formation.quietZone}
        </div>
      </div>
    </article>
  );
