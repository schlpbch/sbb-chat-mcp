...
export default function ChatPanel({ language, isOpen, onClose }: ChatPanelProps) {
 ...
 const { messages, isStreaming, sendMessage, abortStream } = useStreamingChat(language);
 ...
 tabIndex={-1}
 className={`fixed right-6 top-24 h-[calc(100vh-8rem)] w-[400px] backdrop-blur-xl bg-white/80 dark:bg-gray-900/90 rounded-[32px] shadow-2xl z-50 flex flex-col border border-white/40 dark:border-gray-700/40 ring-1 ring-black/5 dark:ring-white/5 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) transform ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0 pointer-events-none'}`}
 >
 {/* Header */}
 ...
 <div>
 <h2 id="chat-title" className="text-white font-bold text-base tracking-tight uppercase leading-none">
 Swiss Companion
 </h2>
 <span className="text-[9px] text-white/80 font-medium uppercase tracking-[0.15em]">Always at your service</span>
 </div>
 </div>
 ...
 <h3 className="text-midnight dark:text-white font-bold text-xl mb-2 tracking-tight">Grüezi!</h3>
 ...
 <StreamingChatMessage key={message.id} message={message} language={language} />
 ))}
