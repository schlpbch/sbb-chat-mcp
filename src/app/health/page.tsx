    ...
    <div className="flex flex-col min-h-screen bg-linear-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="flex-1 pt-20 px-4 sm:px-6 lg:px-8 pb-12">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Overall Status Card */}
          {loading ? (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 mb-6 animate-pulse">
              <div className="h-24 bg-gray-200 dark:bg-gray-700 rounded"></div>
            </div>
          ) : healthData ? (
            <div className={`rounded-2xl shadow-lg p-8 mb-6 border-2 ${
              healthData.status === 'healthy' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' :
              healthData.status === 'degraded' ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20' :
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-700">
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Connection Status</span>
                  <div className="flex items-center space-x-2">
                ...
                {healthData.checks.mcp.serverUrl && (
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4">
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Server URL</span>
                    <code className="block mt-2 text-sm text-gray-900 dark:text-white font-mono break-all">
                      {healthData.checks.mcp.serverUrl}
                    </code>
                  </div>
                )}
                  ...
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4">
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-red-700 dark:text-red-400 mt-1">{healthData.checks.mcp.error}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {healthData.checks.mcp.toolsList && healthData.checks.mcp.toolsList.length > 0 && (
                  <details className="group">
                    <summary className="cursor-pointer text-sm font-semibold text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                      <div className="flex items-center space-x-2">
                        ...
                        <div key={idx} className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                          ...
                          {tool.inputSchema && (
                            <details className="mt-2">
                              <summary className="text-xs text-blue-600 dark:text-blue-400 cursor-pointer hover:underline">View Schema</summary>
                ...
                {healthData.checks.mcp.promptsList && healthData.checks.mcp.promptsList.length > 0 && (
                  <details className="group">
                    <summary className="cursor-pointer text-sm font-semibold text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                      <div className="flex items-center space-x-2">
                    ...
                    <div className="mt-3 space-y-2 max-h-96 overflow-y-auto">
                      {healthData.checks.mcp.promptsList.map((prompt: any, idx: number) => (
                        <div key={idx} className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                          <h5 className="font-semibold text-gray-900 dark:text-white">{prompt.name}</h5>
                          {prompt.description && (
                            ...
                            <div className="mt-2">
                              ...
                              <div className="mt-1 flex flex-wrap gap-2">
                                {prompt.arguments.map((arg: any, argIdx: number) => (
                                  <span key={argIdx} className="inline-flex items-center px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 text-xs rounded">
                ...
                {healthData.checks.mcp.resourcesList && healthData.checks.mcp.resourcesList.length > 0 && (
                  <details className="group">
                    <summary className="cursor-pointer text-sm font-semibold text-gray-700 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400 flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                      <div className="flex items-center space-x-2">
                    ...
                    <div className="mt-3 space-y-2 max-h-96 overflow-y-auto">
                      {healthData.checks.mcp.resourcesList.map((resource: any, idx: number) => (
                        <div key={idx} className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h5 className="font-semibold text-gray-900 dark:text-white">{resource.name}</h5>
                              {resource.description && (
                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{resource.description}</p>
                              )}
                              {resource.uri && (
                                <code className="block mt-2 text-xs text-gray-600 dark:text-gray-400 font-mono">{resource.uri}</code>
                              )}
                            </div>
                          </div>
                          {resource.mimeType && (
                            <span className="inline-block mt-2 px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 text-xs rounded">
                              {resource.mimeType}
                            </span>
                          )}
                        </div>
                ...
                {healthData.checks.mcp.components && Object.keys(healthData.checks.mcp.components).length > 0 && (
                  <details className="group">
                    <summary className="cursor-pointer text-sm font-semibold text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
