...
function ChatContent() {
  ...
  } = useGeolocation(false); // Don't auto-request location

  // Don't show automatic location banner - only request on-demand
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-gray-800 shadow-2xl overflow-hidden transition-colors duration-300">
            ...
            >
              {/* Location Request Button (when not enabled) */}
              {!location && permissionState === 'prompt' && (
                <div className="flex justify-center mb-4">
                  <button
                    onClick={requestLocation}
                    disabled={locationLoading}
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 text-sm font-medium hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <svg
                      className={`h-4 w-4 ${
                    ...
                    <span>
                      {locationLoading
                        ...
                        : 'Enable Location for "from here" queries'}
                    </span>
                  </button>
                </div>
              )}
